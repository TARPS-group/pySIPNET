...
A substance enters a conversion the same way, as an argument beside the unit string. `pysipnet.units.conversion_factor` takes a `units` and a `constituent` for each side and supplies the chemistry Pint cannot: mass and amount convert through `MOLAR_MASS`, a depth or volume of water and a mass through `DENSITY`, and a change of constituent only for a pair in `ATOMS_PER_MOLECULE` (C–CO2, C–CH4, N–N2O), applied on an amount basis. The constituent qualifies the first unit in the string, the position `format_units` prints it in, so `"nmol g-1 s-1"` of CO2 is CO2 per gram of leaf and a `"Pa"` of C cannot become a mass of C. The same reading guards the plain Pint conversion: when the first unit changes kind (an amount to a mass, a mass to a volume) the conversion needs a property of the substance even if Pint sees matching dimensions, so a mole fraction does not silently become a mass fraction by a factor of 1. Every other pairing is refused rather than guessed. `convert_dataarray_units` reads the source units and constituent from a `DataArray`'s attributes rather than taking them as arguments, so a caller cannot state units the data is not in; `convert_units` is the lower-level version for unlabeled values, and refuses anything that carries a `units` attribute of its own. See [Running a Model](user-guide/running-a-model.md#converting-units) for examples.
...
Plain xarray arithmetic drops all of these attributes, so `pysipnet.arithmetic` multiplies, divides, adds and subtracts labeled arrays and writes the `units`, `constituent` and `kind` that are true of the result. The unit and constituent half is `pysipnet.units`' (`product_units`, `quotient_units`, `sum_units`, `difference_units`, on bare strings): the operand carrying the constituent leads the combined unit string, so the constituent still qualifies its first unit, and a combination that would cancel or change that unit is refused. Only two changes of kind are defined, recorded in `KIND_AFTER_TIME_POWER`: a total over the step divided by the step length is a rate, and a rate times it is a total. See [Running a Model](user-guide/running-a-model.md#combining-variables).
